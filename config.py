...
# === Quality mapping (qx → Bilibili qn) ===
...
DYNAMIC_SETTINGS: dict[str, str] = {
    ...
    "api_token": "please_change_this_token",
    "api_port": "14515",
    "web_port": "8080",
    "enable_web_auth": "1",
...
DB_PATH = os.path.join(DATA_DIR, "bilibili_proxy.db")
...
async def set_dynamic_setting(key: str, value: str) -> None:
    ...
    if key == "api_port" and value.isdigit():
        global API_PORT
        API_PORT = int(value)
        # Can't hot-reload the server port, requires restart
    if key == "web_port" and value.isdigit():
        global WEB_PORT
        WEB_PORT = int(value)
...
async def init_config() -> None:
    """Initialize config on startup: load dynamic settings from DB."""
